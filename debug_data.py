#!/usr/bin/env python
"""
Скрипт для проверки генерации и фильтрации данных в приложении дашборда
"""
import pandas as pd
import numpy as np
...
print("Группы KPI:")
print(df_yearly['Название'].unique())

print("\nПроверяем фильтрацию по месяцу...")
# Выберем первый доступный месяц
if not df_cleaned.empty:
    # Проверим типы данных в столбце Дата_Начала_DT
    print(f"Тип данных в 'Дата_Начала_DT': {df_cleaned['Дата_Начала_DT'].dtype}")
    print(f"Первые значения 'Дата_Начала_DT': {df_cleaned['Дата_Начала_DT'].head()}")
    
    # Используем оригинальный столбец Дата_Начала для получения месяца
    first_month = df_cleaned['Дата_Начала'].dt.strftime('%Y-%m').iloc[0]
    print(f"Используем месяц: {first_month}")
    df_monthly = filter_data_by_period(df_cleaned, "Месяц (по неделям)", first_month)
    print(f"Фильтрация по месяцу: {len(df_monthly)} записей")
    print("Группы KPI:")
    print(df_monthly['Название'].unique())
else:
