...
st.set_page_config(page_title="АНО «Синяя птица» - KPI Monitor v2.11", layout="wide")
...
def clean_data_types(df):
    ...
    # 1. Приведение даты к Python date object. (НаT, если не удается)
    df['Дата_Начала'] = pd.to_datetime(df['Дата_Начала'], errors='coerce').dt.date

    # 2. Приведение числовых колонок к float. (NaN, если не удается)
    for col in numerical_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce')

    # 3. Удаление строк, где отсутствуют ключевые параметры (KPI_ID и числовые).
    # Это предотвратит ошибки при группировке.
